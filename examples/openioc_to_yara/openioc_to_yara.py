...
%(strings)s'''
    ...
    def emit_yara(self):
        ...
        for iocid in self.iocs:
            ...
            if (not condition_string) and (not embedded_signatures):
                ...
                mapping['meta'] = self.YARA_META_TEMPLATE % {'meta' : metadata_string}
                mapping['strings'] = self.YARA_STRINGS_TEMPLATE % {'strings' : strings_list}
                mapping['condition'] = self.YARA_CONDITION_TEMPLATE % {'condition' : condition_string}
                if strings_list:
    ...
    def get_yara_condition(self, iocid):
        ...
        for elem in ioc_obj.top_level_indicator.xpath('.//IndicatorItem[Context/@document = "Yara" and Context/@search != "Yara/Yara"]'):
            current = elem
        ...
        condition_string = self.get_yara_condition_string(ioc_obj.top_level_indicator, ioc_obj.parameters, ids_to_process)
        return condition_string
        
    def get_yara_condition_string(self, indicator_node, parameters_node, ids_to_process, condition_string = '', joining_value = 'or'):
        '''
        ...
        if indicator_node_id not in ids_to_process:
            raise IOCParseError('Entered into get_yara_condition_string with a invalid node to walk [%s]' % (indicator_node_id))
        ...
        for param in parameters_node.xpath('.//param[@ref-id="%s"]' % (indicator_node_id)):
            ...
            if node.tag == 'IndicatorItem':
                ...
                if is_set:
                    ...
                    if not yara_condition:
                        raise IOCParseError('Invalid IndicatorItem condition encountered [%s][%s]' % (str(node_id), str(condition)))
                    ...
                    # handle parameters
                        ...
                        params = parameters_node.xpath('.//param[@ref-id="%s" and (@name="yara/count" or @name="yara/offset/at" or @name="yara/offset/in")]' % (node_id))
                        if len(params) > 1:
                            raise IOCParseError('More than one condition parameters assigned to IndicatorItem [%s]' % (node_id))
                        for param in params:
                ...
                if (not parameters_node.xpath('.//param[@ref-id="%s" and @name="yara/set"]' % (node_id))) and has_siblings(node):
    ...
    def get_yara_stringlist(self, iocid):
        ...
        for node in ioc_obj.top_level_indicator.xpath('.//IndicatorItem[Context/@search = "Yara/HexString" or Context/@search = "Yara/TextString" or Context/@search = "Yara/RegexString"]'):
            ...
            params = ioc_obj.parameters.xpath('.//param[@ref-id="%s" and (@name="yara/wide" or @name="yara/ascii" or @name="yara/fullword")]' % str(node_id))
...
def oty_options():
    ...
    opts.append(optparse.make_option('--output', '-o', dest = 'output', 
                                    help = 'File to write yara signatures too.  This will overwrite an existing file.  By default, this is "iocs.yara"', default = None))
